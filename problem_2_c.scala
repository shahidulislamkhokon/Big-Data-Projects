...
///home/users/mdislam/myfile/covtype.data
//val data = sc.textFile("myfile/NOAA-LU-1949-2020")
val data = sc.textFile("/home/users/mrahman/myfile/NOAA-LU-1949-2020")

data.take(10).foreach(println)

val filteredData = data.filter(line => line.substring(87, 92) != "+9999") // filter lines with missing temperature label!
...
// subset of dataset used for training
trainData.cache()
// subset of dataset used for final evaluation ("testing") 
testData.cache() 
...
predictions.show(5)
val evaluator = new MulticlassClassificationEvaluator().setLabelCol("airTemperature").setFeaturesCol("features").setPredictionCol("prediction").setMetricName("accuracy")
...
println("Mean Squared Error (MSE) = " + testMSE)